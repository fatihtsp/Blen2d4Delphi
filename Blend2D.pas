...
type
  ...
  public
    ...
    property W: Double read FHandle.w write FHandle.w;
    property H: Double read FHandle.h write FHandle.h;
    property Handle: BLRoundRect read FHandle;
  end;
...
type
  ...
  public
    ...
    constructor Create(const AType: TBLGradientType; const AValues: PDouble = nil); overload;
...
type
  ...
  protected
    ...
    procedure Initialize(const AWidth, AHeight: Integer; const AFormat: TBLFormat);
...
{ ============================================================================
  ...
  TBLImageCodecFeature = (
    Read       = 0, // Image codec supports reading images (can create IBLImageDecoder).
    Write      = 1, // Image codec supports writing images (can create IBLImageEncoder).
    Lossless   = 2, // Image codec supports lossless compression. //
    Lossy      = 3, // Image codec supports lossy compression. //
    MultiFrame = 4, // Image codec supports writing multiple frames (GIF).
    IPTC       = 28,// Image codec supports IPTC metadata. //
    EXIF       = 29,// Image codec supports EXIF metadata. //
    XMP        = 30// Image codec supports XMP metadata. //
    );
...
type
  ...
  ['{0B892B22-C3B5-434D-854C-FBD0FD431B7B}']
    ...
    function GetGlyphRun: _PBLGlyphRun;
    ...
    property GlyphRun: _PBLGlyphRun read GetGlyphRun;
...
type
  ...
  protected
    ...
    procedure InitializeFromFile(const AFilename: String; const AReadFlags: TBLFileReadFlags = []);
    ...
    function QueryTables(const AFaceIndex: Integer; const ATags: TArray<TBLTag>;
      out ATables: TArray<TBLFontTable>): Integer;
  private
    constructor Create(const AHandle: BLFontDataCore; const AOwnsHandle: Boolean); overload;
  {$ENDREGION 'Internal Declarations'}
  public
    constructor Create; overload;
    destructor Destroy; override;
...
type
  ...
  ['{D51BEBAD-EB2A-41CA-B76C-5773E696CA80}']
    ...
    function GetPanose: BLFontPanose;//TBLFontPanose;
    ...
    procedure InitializeFromData(const AFontData: IBLFontData; const AFaceIndex: Integer);
    ...
    property Panose: BLFontPanose read GetPanose;//TBLFontPanose read GetPanose;
...
type
  ...
  ['{47ED75E4-8E6C-4D0E-95D9-6321CD4F8694}']
    ...
    procedure InitializeFromFace(const AFace: IBLFontFace; const ASize: Single);
...
type
  ...
  private
    ...
    FSinkPath: IBLPath;
  private
    class function DoSink(path: PBLPathCore; info: Pointer; closure: Pointer): Cardinal; cdecl; static;
  protected
    { IBLFont }
    function GetIsNone: Boolean;
    function GetFaceType: TBLFontFaceType;
...
type
  ...
  ['{079F0FF5-12D6-4D53-B5AA-79C1AEE83068}']
    ...
    function GetState: TBLContextState;
    {$ENDREGION 'Internal Declarations'}
    ...
    procedure Start(const AImage: IBLImage; const AOptions: TBLContextCreateInfo); overload;
...
//    procedure FillRegion(const ARegion: IBLRegion);
    ...
    procedure FillGlyphRun(const ADst: TBLPointI; const AFont: IBLFont; const AGlyphRun: TBLGlyphRun); overload;
    procedure FillGlyphRun(const ADst: TBLPoint; const AFont: IBLFont;  const AGlyphRun: TBLGlyphRun); overload;
    procedure FillGlyphRun(const ADst: TBLPointI; const AFont: IBLFont; const AGlyphRun: PBLGlyphRun); overload;
    procedure FillGlyphRun(const ADst: TBLPoint; const AFont: IBLFont; const AGlyphRun: _PBLGlyphRun); overload;
    procedure FillGlyphRun(const ADst: TBLPoint; const AFont: IBLFont;  const AGlyphRun: PBLGlyphRun); overload;
    ...
    procedure StrokeGlyphRun(const ADst: TBLPointI; const AFont: IBLFont; const AGlyphRun: TBLGlyphRun); overload;
    procedure StrokeGlyphRun(const ADst: TBLPoint; const AFont: IBLFont; const AGlyphRun: TBLGlyphRun); overload;
    ...
    { Blits an area of source image ASrc specified by ASrcArea scaled to fit
      into ADst rectangle. }
    procedure BlitImage(const ADst: TBLRect; const ASrc: IBLImage; const ASrcArea: TBLRectI); overload;
    procedure BlitImage(const ADst: TBLRectI; const ASrc: IBLImage; const ASrcArea: TBLRectI); overload;

    Property State: TBLContextState read GetState;
...
//    procedure FillRegion(const ARegion: IBLRegion);
    ...
    procedure FillText(const ADst: TBLPointI; const AFont: IBLFont; const AText: String); overload;
    procedure FillText(const ADst: TBLPoint; const AFont: IBLFont; const AText: String); overload;
    procedure FillText(const ADst: TBLPointI; const AFont: IBLFont; const AText: UTF8String); overload;
    procedure FillText(const ADst: TBLPoint; const AFont: IBLFont; const AText: UTF8String); overload;
    procedure FillText(const ADst: TBLPointI; const AFont: IBLFont; const AText: UCS4String); overload;
    procedure FillText(const ADst: TBLPoint; const AFont: IBLFont; const AText: UCS4String); overload;
...
type
  ...
  public
    ...
    property Version: Cardinal read FHandle.patchVersion;//FHandle.version.version;

    { Decomposed Blend2D version }
    property PatchVersion: UInt32 read FHandle.patchVersion;
    property MinorVersion: UInt32 read FHandle.minorVersion;
    property MajorVersion: UInt32 read FHandle.majorVersion;
    ...
    property MaxImageSize: UInt32 read FHandle.maxImageSize;

    { Maximum number of threads for asynchronous operations, including
      rendering. }
    property MaxThreadCount: UInt32 read FHandle.maxThreadCount;
  end;
...
type
  ...
  public
    ...
    property AllocationGranularity: UInt32 read FHandle.allocationGranularity;
  end;
...
type
  ...
  protected
    ...
    procedure RevokeOwnership;
  public
    constructor Create(const AType: BLObjectType);
    destructor Destroy; override;
  end;
...
type
  ...
  public
    class function ArrayElementType<T>: BLObjectType; static;
...
constructor TBLArray.Create(const AType: BLObjectType);
begin
  inherited Create;
  FHandle := ftNewblArrayCore()^;
...
class function TBLUtils.ArrayElementType<T>: BLObjectType;
var
  Info: PTypeInfo;
  Data: PTypeData;
  ...
  case GetTypeKind(T) of
    ...
    tkEnumeration:
      case GetTypeData(Info)^.OrdType of
        otSByte: Exit(BL_OBJECT_TYPE_ARRAY_INT8);
        otUByte: Exit(BL_OBJECT_TYPE_ARRAY_UINT8);
        otSWord: Exit(BL_OBJECT_TYPE_ARRAY_INT16);
        otUWord: Exit(BL_OBJECT_TYPE_ARRAY_UINT16);
        otSLong: Exit(BL_OBJECT_TYPE_ARRAY_INT32);
        otULong: Exit(BL_OBJECT_TYPE_ARRAY_UINT32);
      end;
    ...
    tkInt64:
      begin
        ...
        if (Data^.MaxInt64Value > Data^.MinInt64Value) then
          Exit(BL_OBJECT_TYPE_ARRAY_INT64)
        else
          Exit(BL_OBJECT_TYPE_ARRAY_UINT64);
      end;
    ...
    tkFloat:
      begin
        ...
        case Data^.FloatType of
          ftSingle  : Exit(BL_OBJECT_TYPE_ARRAY_FLOAT32);
          ftDouble  : Exit(BL_OBJECT_TYPE_ARRAY_FLOAT64);
        end;
      end;

    tkRecord:
      case SizeOf(T) of
         1: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_1);
         2: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_2);
         3: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_3);
         4: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_4);
         6: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_6);
         8: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_8);
        10: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_10);
        12: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_12);
        16: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_16);
        20: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_20);
        24: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_24);
        32: Exit(BL_OBJECT_TYPE_ARRAY_STRUCT_32);
      end;
  end;
...
begin
  ...
  if (Count <> 0) then begin
    ...
    _BLCheck(blArrayAppendData(A, @AArray[0], Count));
  end;
end;
...
var
  ...
  data: Pointer;
begin
  //Impl := AArray.Data;//AArray.impl;
  //Assert(Assigned(Impl) and (Impl.itemSize = SizeOf(T)));
  Count := blArrayGetSize(@AArray);//Impl.size;
  if (Count = 0) then Exit(nil);

  SetLength(Result, Count);
  data := blArrayGetData(@AArray);
  Move(data^, Result[0], Count * SizeOf(T));
end;
...
begin
  ...
  Count := blArrayGetSize(AArray.Handle);//Impl.size;
  if (Count = 0) then Exit(nil);

  SetLength(Result, Count);
  Move(blArrayGetData(AArray.Handle)^, Result[0], Count * SizeOf(T));
end;
...
class operator TBLPointI.Add(const ALeft: Integer; const ARight: TBLPointI): TBLPointI;
begin
  Result.Reset(ALeft + ARight.FHandle.x, ALeft + ARight.FHandle.y);
end;

class operator TBLPointI.Add(const ALeft: TBLPointI; const ARight: Integer): TBLPointI;
begin
...
begin
  Result := (FHandle.x = AOther.FHandle.x) and (FHandle.y = AOther.FHandle.y);
end;

class operator TBLPointI.IntDivide(const ALeft: TBLPointI; const ARight: Integer): TBLPointI;
begin
  Result.Reset(ALeft.FHandle.x div ARight, ALeft.FHandle.y div ARight);
end;

class operator TBLPointI.IntDivide(const ALeft: Integer; const ARight: TBLPointI): TBLPointI;
begin
...
constructor TBLGradient.Create(const AType: TBLGradientType; const AValues: PDouble);
begin
  inherited Create;
  FHandle := ftNewblGradientCore()^;
  _BLCheck(blGradientInitAs(@FHandle, Ord(AType), AValues, BL_EXTEND_MODE_PAD, nil, 0, nil));
end;
...
begin
  Result := PBLGradientStop(@FHandle)<>Nil;//(FHandle.impl.stops.size = 0);
end;
...
begin
  _BLCheck(blGradientRemoveStopsByIndex(@FHandle, ARange.FHandle.start, ARange.FHandle.&end));
end;

procedure TBLGradient.RemoveStopsByOffset(const AOffsetMin, AOffsetMax: Double);
begin
  _BLCheck(blGradientRemoveStopsByOffset(@FHandle, AOffsetMin, AOffsetMax));
end;
...
procedure TBLGradient.SetValue(const AIndex: TBLGradientValue; const AValue: Double);
begin
...
begin
  pd[0] := AValues.X0; pd[1] := AValues.y0; pd[2] := AValues.X1; pd[3] := AValues.y1;
  _BLCheck(blGradientSetValues(@FHandle, 0, PDouble(@pd), 4 ));
end;
...
procedure TBLGradient.SetValues(const AIndex: TBLGradientValue; const AValues: TArray<Double>);
begin
...
begin
  Result := blArrayGetCapacity(@FHandle)  ;//FHandle.impl.capacity;
end;
...
begin
  Result := Pointer( blPathGetCommandData(@FHandle) );//Pointer(FHandle.impl.commandData);
end;

function TBLPath.GetCommandDataEnd: PBLPathCmd;
begin
  Result := Pointer(blPathGetCommandData(@FHandle) + blPathGetSize(@FHandle)) ;//Pointer(FHandle.impl.commandData + FHandle.impl.size);
end;
...
begin
  Result := blPathGetSize(@FHandle);//FHandle.impl.size;
end;
...
begin
  Result := (blPathGetSize(@FHandle)=0);//(FHandle.impl.size = 0);
end;

function TBLPath.GetIsNone: Boolean;
begin
  Result := (PBlPathCore(@FHandle)<>Nil); //((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;
...
begin
  Result := Pointer(blPathGetVertexData(@FHandle)) ;//Pointer(FHandle.impl.vertexData);
end;

function TBLPath.GetVertexDataEnd: PBLPoint;
begin
  Result := Pointer(blPathGetVertexData(@FHandle) + blPathGetSize(@FHandle)); //Pointer(FHandle.impl.vertexData + FHandle.impl.size);
end;

function TBLPath.GetView: TBLPathView;
begin
   ftGetPathCoreView(@FHandle, @Result.FHandle);//FHandle.impl.view;
end;
...
constructor TBLImage.Create(const AWidth, AHeight: Integer; const AFormat: TBLFormat);
begin
  inherited Create;
  FHandle := ftNewblImageCore()^;
...
var
 blID: _PBLImageData;
begin
  blImageGetData(@FHandle, blID);
  Result := TBLFormat(blID^.format);
end;
...
begin
  Result := (GetFormat = TBLFormat(BL_FORMAT_NONE));//(FHandle.impl.format = BL_FORMAT_NONE);
end;

function TBLImage.GetIsNone: Boolean;
begin
  Result := (PBlImageCore(@FHandle)<>Nil);//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;
...
begin
  blImageGetData(@FHandle, blID);
  Result := TBLSizeI(blID^.size);//TBLSizeI(FHandle.impl.size);
end;
...
begin
  blImageGetData(@FHandle, blID);
  Result := blID^.size.w; //Result := FHandle.impl.size.w;
end;

procedure TBLImage.Initialize(const AWidth, AHeight: Integer; const AFormat: TBLFormat);
begin
  //if ( PBLImageCore(@FHandle) = Nil ) then FHandle := ftNewblImageCore()^;
...
begin
  if Assigned(AOnDestroy) then begin
...
procedure TBLImage.ReadFromData(const AData: Pointer; const ASize: Integer; const ACodecs: TArray<IBLImageCodec>);
var
...
begin
  ...
  for I := 0 to Length(CodecHandles) - 1 do begin
...
procedure TBLImage.ReadFromData(const AView: TBLArrayView<Byte>; const ACodecs: TArray<IBLImageCodec>);
begin
...
begin
  if (ACodec = nil) then Codec := nil else Codec := ACodec.Handle;
...
procedure TBLImage.WriteToFile(const AFilename: String; const ACodec: IBLImageCodec);
var
  Codec: PBLImageCodecCore;
...
begin
  inherited Create;
  FHandle := ftNewblImageDecoderCore()^;
...
begin
  ...
  //ftFreeblImageDecoderCore(@FHandle);
  inherited;
end;
...
begin
  if (Obj = nil) then
    ...
    Result := (@FHandle = @TBLImageDecoder(Obj).FHandle)
  else
    Result := False;
end;
...
begin
  if (AOther = nil) then
    ...
    Result := (@FHandle = AOther.Handle);
end;

function TBLImageDecoder.GetBufferIndex: NativeInt;
begin
  Result := 0;//FHandle.impl.bufferIndex;  //Not implemented due to changes in blend2D, so write helper function in blend2d
end;

function TBLImageDecoder.GetFrameIndex: Integer;
begin
  Result := 0;//FHandle.impl.frameIndex;  //Not implemented due to changes in blend2D, so write helper function in blend2d
end;
...
begin
  Result := (PBLImageDecoderCore(@FHandle)<>Nil) ;//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;

function TBLImageDecoder.GetLastResult: TBLResultCode;
begin
  Result := TBLResultCode(ftGetblLastDecoderResult(@FHandle));//TBLResultCode(FHandle.impl.lastResult);
end;

function TBLImageDecoder.ReadFrame(const ABuffer: Pointer; const ASize: Integer): IBLImage;
begin
  Result := TBLImage.Create;
...
begin
  Result := TBLImage.Create;
  _BLCheck(blImageDecoderReadFrame(@FHandle, Result.Handle, ABuffer.FHandle.data, ABuffer.FHandle.size));
end;

function TBLImageDecoder.ReadFrame(const ABuffer: TBytes): IBLImage;
begin
  Result := TBLImage.Create;
  _BLCheck(blImageDecoderReadFrame(@FHandle, Result.Handle, Pointer(ABuffer), Length(ABuffer)));
end;

procedure TBLImageDecoder.ReadInfo(const ABuffer: Pointer; const ASize: Integer; out AInfo: TBLImageInfo);
begin
  _BLCheck(blImageDecoderReadInfo(@FHandle, @AInfo.FHandle, ABuffer, ASize));
end;

procedure TBLImageDecoder.ReadInfo(const ABuffer: TBLArrayView<Byte>; out AInfo: TBLImageInfo);
begin
  _BLCheck(blImageDecoderReadInfo(@FHandle, @AInfo.FHandle,
    ABuffer.FHandle.data, ABuffer.FHandle.size));
end;

procedure TBLImageDecoder.ReadInfo(const ABuffer: TBytes; out AInfo: TBLImageInfo);
begin
  _BLCheck(blImageDecoderReadInfo(@FHandle, @AInfo.FHandle, Pointer(ABuffer), Length(ABuffer)));
end;
...
begin
  inherited Create;
  FHandle := ftNewblImageEncoderCore()^;
...
begin
  ...
  //ftFreeblImageEncoderCore(@FHandle);
  inherited;
end;
...
begin
  if (Obj = nil) then
    ...
    Result := (@FHandle = TBLImageEncoder(Obj).GetHandle)
  else
    Result := False;
end;
...
begin
  if (AOther = nil) then
    ...
    Result := (@FHandle = AOther.GetHandle);
end;

function TBLImageEncoder.GetBufferIndex: NativeInt;
begin
  Result := 0;//FHandle.impl.bufferIndex;
end;

function TBLImageEncoder.GetFrameIndex: Integer;
begin
  Result := 0;//FHandle.impl.frameIndex;
end;
...
begin
  Result := (PBLImageEncoderCore(@FHandle) <> Nil);//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;

function TBLImageEncoder.GetLastResult: TBLResultCode;
begin
  Result := TBLResultCode(ftGetblLastEncoderResult(@FHandle));//TBLResultCode(FHandle.impl.lastResult);
end;
...
begin
  inherited Create;
  FHandle := ftNewblImageCodecCore()^;
  blImageCodecInit(@FHandle);
  FOwnsHandle := True;
end;

constructor TBLImageCodec.Create(const AHandle: BLImageCodecCore; const AOwnsHandle: Boolean);
begin
  inherited Create;
  FHandle     := AHandle;
...
begin
  if (FOwnsHandle) then begin
    ...
    //ftFreeblImageCodecCore(@FHandle);
  end;
  inherited;
end;
...
begin
  if (AOther = nil) then
    ...
    Result := (@FHandle = AOther.Handle) ;//(FHandle.impl = AOther.Handle.impl);
end;
...
begin
  if (Obj = nil) then
    ...
    Result := (@FHandle = TBLImageCodec(Obj).GetHandle)
  else
    Result := False;
end;

function TBLImageCodec.FindByData(const AData: TBytes; const ACodecs: TArray<IBLImageCodec>): Boolean;
begin
  Result := FindByData(Pointer(AData), Length(AData), ACodecs);
end;

function TBLImageCodec.FindByData(const AData: Pointer; const ASize: Integer): Boolean;
var
  Res: BLResultCode;
begin
...
begin
  Result := String(UTF8String(ftGetblImageCodecExtensions(@FHandle)));//String(UTF8String(FHandle.impl.extensions));
end;

function TBLImageCodec.GetFeatures: TBLImageCodecFeatures;
begin
  Result := TBLImageCodecFeatures(Cardinal(ftGetblImageCodecFeatures(@FHandle)));//TBLImageCodecFeatures(FHandle.impl.features);
end;
...
begin
  Result := (PBLImageCodecCore(@FHandle)<>Nil) ;//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;

function TBLImageCodec.GetMimeType: String;
begin
  Result := String(UTF8String(ftGetblImageCodecmimeType(@FHandle)));//String(UTF8String(FHandle.impl.mimeType));
end;

function TBLImageCodec.GetName: String;
begin
  Result := String(UTF8String(ftGetblImageCodecName(@FHandle)));//String(UTF8String(FHandle.impl.name));
end;

function TBLImageCodec.GetVendor: String;
begin
  Result := String(UTF8String(ftGetblImageCodecVendor(@FHandle)));//String(UTF8String(FHandle.impl.vendor));
end;
...
var
 ft: NativeUInt;
begin
  case Ord(AFeature) of
  0: ft := BL_IMAGE_CODEC_FEATURE_READ;
  1: ft := BL_IMAGE_CODEC_FEATURE_WRITE;
  3: ft := BL_IMAGE_CODEC_FEATURE_LOSSLESS;
  4: ft := BL_IMAGE_CODEC_FEATURE_LOSSY;
  5: ft := BL_IMAGE_CODEC_FEATURE_MULTI_FRAME;
  6: ft := BL_IMAGE_CODEC_FEATURE_IPTC;
  7: ft := BL_IMAGE_CODEC_FEATURE_EXIF;
  8: ft := BL_IMAGE_CODEC_FEATURE_XMP;
  end;
  Result := (ftGetblImageCodecHasFeature(@FHandle, ft));//;//((FHandle.impl.features and Ord(AFeature)) <> 0);
end;
...
begin
  inherited;
  FHandle := ftNewblPatternCore()^;
  blPatternInit(@FHandle);
end;

constructor TBLPattern.Create(const AImage: IBLImage; const AExtendMode: TBLExtendMode);
begin
  inherited Create;
  FImage  := AImage;
  FHandle := ftNewblPatternCore()^;
...
constructor TBLPattern.Create(const AImage: IBLImage; const AExtendMode: TBLExtendMode; const AMatrix: TBLMatrix2D);
begin
  inherited Create;
  FImage := AImage;
  FHandle := ftNewblPatternCore()^;
...
constructor TBLPattern.Create(const AImage: IBLImage; const AArea: TBLRectI; const AExtendMode: TBLExtendMode);
begin
  inherited Create;
  FImage  := AImage;
  FHandle := ftNewblPatternCore()^;
...
begin
  blPatternGetArea(@Self.FHandle, @Result.FHandle); //Result.FHandle := FHandle.impl.area;
end;

function TBLPattern.GetExtendMode: TBLExtendMode;
begin
  Result := TBLExtendMode( blPatternGetExtendMode(@FHandle) ); //TBLExtendMode(FHandle.impl.extendMode);
end;
...
begin
  Result := ftGetblPatternHasMatrix(@FHandle); //(FHandle.impl.matrixType <> BL_MATRIX2D_TYPE_IDENTITY);
end;
...
begin
  Result := (PBLPatternCore(@FHandle) <> Nil);//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;

function TBLPattern.GetMatrix: TBLMatrix2D;
begin
  blPatternGetMatrix(@FHandle, @Result.FHandle); //Result.FHandle := FHandle.impl.matrix;
end;

function TBLPattern.GetMatrixType: TBLMatrix2DType;
begin
  Result := TBLMatrix2DType(blPatternGetMatrixType(@FHandle));//Result := TBLMatrix2DType(FHandle.impl.matrixType);
end;
...
procedure TBLFontUnicodeCoverage.ClearBit(const AIndex: TBLFontUnicodeCoverageIndex);
var
  I: Integer;
begin
  I := Ord(AIndex);
...
class operator TBLFontUnicodeCoverage.Equal(const ALeft, ARight: TBLFontUnicodeCoverage): Boolean;
begin
...
function TBLFontUnicodeCoverage.GetBit(const AIndex: TBLFontUnicodeCoverageIndex): Boolean;
var
  I: Integer;
begin
  I := Ord(AIndex);
...
class operator TBLFontUnicodeCoverage.NotEqual(const ALeft, ARight: TBLFontUnicodeCoverage): Boolean;
begin
  Result := not (ALeft = ARight);
end;
...
procedure TBLFontUnicodeCoverage.SetBit(const AIndex: TBLFontUnicodeCoverageIndex; const AValue: Boolean);
begin
  if (AValue) then
    SetBit(AIndex)
  else
...
procedure TBLFontUnicodeCoverage.SetBit(const AIndex: TBLFontUnicodeCoverageIndex);
var
  I: Integer;
begin
  I := Ord(AIndex);
...
begin
  inherited;
  FHandle := ftNewblGlyphBufferCore()^;
...
function TBLGlyphBuffer.GetGlyphRun: _PBLGlyphRun;
begin
  Result := ftGetblGlyphBufferRun(@FHandle) ;//@FHandle.impl.data.glyphRun;
end;
...
procedure TBLGlyphBuffer.SetGlyphs(const AGlyphData: Pointer; const ASize, AGlyphIdSize, AGlyphAdvance: Integer);
begin
  _BLCheck(blGlyphBufferSetGlyphsFromStruct(@FHandle, AGlyphData, ASize, AGlyphIdSize, AGlyphAdvance));
end;

procedure TBLGlyphBuffer.SetGlyphs(const AGlyphData: PCardinal; const ALength: Integer);
begin
...
procedure TBLGlyphBuffer.SetText(const AText: Pointer; const ALength: Integer; const AEncoding: TBLTextEncoding);
begin
...
begin
  inherited;
  //var ZHandle : PBLFontDataCore := ftNewblFontDataCore();
  FHandle := PBLFontDataCore(ftNewblFontDataCore())^;
  blFontDataInit(@FHandle);
  FOwnsHandle := True;
end;

constructor TBLFontData.Create(const AHandle: BLFontDataCore; const AOwnsHandle: Boolean);
begin
  inherited Create;
  FHandle     := PBLFontDataCore(@AHandle)^;
  FOwnsHandle := AOwnsHandle;
end;
...
//BLFontManagerGetFaceCount should be used or not?????
function TBLFontData.GetFaceCount: Integer;
var
 bli: _PblFontFaceInfo;
begin
  blFontFaceGetFaceInfo(@FHandle, bli) ;//FHandle.impl.faceCount;
  Result := 0;//bli^.FaceType;
end;
...
begin
  //blFontFaceGetFaceInfo(@FHandle, bli) ;//FHandle.impl.faceCount;
  //Result := TBLFontFaceType(bli^.faceType);//TBLFontFaceType(FHandle.impl.faceType);
  Result := TBLFontFaceType(ftGetblFontDataFaceType(@FHandle));
end;
...
begin
  //blFontFaceGetFaceInfo(@FHandle, bli) ;//FHandle.impl.faceCount;
  Byte(Result) := ftGetblFontDataFaceFlags(@FHandle);//FHandle.impl.flags;
end;
...
begin
  Result := ((BL_FONT_DATA_FLAG_COLLECTION and Byte(GetFlags)) <> 0);//((FHandle.impl.flags and BL_FONT_DATA_FLAG_COLLECTION) <> 0);
end;

function TBLFontData.GetIsNone: Boolean;
begin
  Result := (PBLFontDataCore(@FHandle) <> Nil);//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;
...
begin
  if (AData <> nil) then begin
    ...
    //if (PBLFontDataCore(@FHandle)<>Nil) then blFontDataReset(@FHandle);
...
procedure TBLFontData.InitializeFromData(const AData: Pointer;
                                         const ASize: Integer;
                                         const AOnDestroy: TBLFontDataDestroyEvent);
var
  DestroyFunc: BLDestroyImplFunc;
  DestroyData: PDestroyData;
...
function TBLFontData.QueryTables(const AFaceIndex: Integer;
                                 const ATags: TArray<TBLTag>;
                                 out ATables: TArray<TBLFontTable>): Integer;
begin
  SetLength(ATables, Length(ATags));
  Result := blFontDataQueryTables(@FHandle, AFaceIndex, Pointer(ATables), Pointer(ATags), Length(ATags));
end;
...
begin
  inherited;
  FHandle := ftNewblFontFaceCore()^;
  //var ZHandle: PBLFontFaceCore := ftNewblFontFaceCore();
  FData   := TBLFontData.Create;
  blFontFaceInit(@FHandle);
end;

destructor TBLFontFace.Destroy;
begin
  //TBLFontData(FData).Free;
  blFontFaceReset(@FHandle);
  blFontFaceDestroy(@FHandle);
  //ftFreeblFontFaceCore(@FHandle);
  inherited;
end;
...
begin
  if (FData = nil) then
    FData := TBLFontData.Create(TBLFontData(Self.FData).FHandle, False);
...
begin
  blFontGetDesignMetrics( @FHandle, @Result.FHandle); //Result.FHandle := //FHandle.impl.designMetrics;
end;

function TBLFontFace.GetDiagFlags: TBLFontFaceDiagFlags;
begin
  //blFontGetDesignMetrics( @FHandle, @(Word(Result).FHandle) ); // Word(Result) := FHandle.impl.faceInfo.diagFlags;
  Word(Result) :=  GetFaceInfo.FHandle.diagFlags;
  //Word(Result) := FHandle. faceInfo.diagFlags;
end;

function TBLFontFace.GetFaceFlags: TBLFontFaceFlags;
begin
  Cardinal(Result) := GetFaceInfo.FHandle.faceFlags; //FHandle.impl.faceInfo.faceFlags;
end;

function TBLFontFace.GetFaceIndex: Integer;
begin
  Result := GetFaceInfo.FHandle.faceIndex;//FHandle.impl.faceInfo.faceIndex;
end;

function TBLFontFace.GetFaceInfo: TBLFontFaceInfo;
begin
  blFontFaceGetFaceInfo(@FHandle, @(Result.FHandle) );//Result.FHandle := FHandle.impl.faceInfo;
end;

function TBLFontFace.GetFaceType: TBLFontFaceType;
begin
  Result := TBLFontFaceType(GetFaceInfo.FHandle.faceType);//TBLFontFaceType(FHandle.impl.faceInfo.faceType);
end;
...
begin
  data := ftGetblFontFaceFamilyName( @FHandle, @Sz);
  SetString(S, data, sz);
  Result := String(S);
end;
...
begin
  data := ftGetblFontFaceFullName( @FHandle, @Sz);
  SetString(S, data, sz);  //  SetString(S, FHandle.impl.fullName.impl.data, FHandle.impl.fullName.impl.size);
...
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_BASELINE_Y_EQUALS_0) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_BASELINE_Y_EQUALS_0) <> 0);
end;

function TBLFontFace.GetHasCharToGlyphMapping: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_CHAR_TO_GLYPH_MAPPING) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_CHAR_TO_GLYPH_MAPPING) <> 0);
end;

function TBLFontFace.GetHasHorizontalKerning: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_HORIZONTAL_KERNING) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_HORIZONTAL_KERNING) <> 0);
end;

function TBLFontFace.GetHasHorizontalMetrics: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_HORIZONTAL_METRICS) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_HORIZONTAL_METRICS) <> 0);
end;

function TBLFontFace.GetHasLSBPointXAt0: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_LSB_POINT_X_EQUALS_0) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_LSB_POINT_X_EQUALS_0) <> 0);
end;

function TBLFontFace.GetHasOpenTypeFeatures: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_OPENTYPE_FEATURES) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_OPENTYPE_FEATURES) <> 0);
end;

function TBLFontFace.GetHasOpenTypeVariations: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_OPENTYPE_VARIATIONS) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_OPENTYPE_VARIATIONS) <> 0);
end;

function TBLFontFace.GetHasPanoseData: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_PANOSE_DATA) <> 0;//(FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_PANOSE_DATA) <> 0);
end;

function TBLFontFace.GetHasTypographicMetrics: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_TYPOGRAPHIC_METRICS) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_TYPOGRAPHIC_METRICS) <> 0);
end;

function TBLFontFace.GetHasTypographicNames: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_TYPOGRAPHIC_NAMES) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_TYPOGRAPHIC_NAMES) <> 0);
end;

function TBLFontFace.GetHasUnicodeCoverage: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_UNICODE_COVERAGE) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_UNICODE_COVERAGE) <> 0);
end;

function TBLFontFace.GetHasVariationSequences: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_VARIATION_SEQUENCES) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_VARIATION_SEQUENCES) <> 0);
end;

function TBLFontFace.GetHasVerticalKerning: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_VERTICAL_KERNING) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_VERTICAL_KERNING) <> 0);
end;

function TBLFontFace.GetHasVerticalMetrics: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_VERTICAL_METRICS) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_VERTICAL_METRICS) <> 0);
end;

function TBLFontFace.GetIsLastResortFont: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_LAST_RESORT_FONT) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_LAST_RESORT_FONT) <> 0);
end;

function TBLFontFace.GetIsNone: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_IMPL_TRAIT_NULL) <> 0;//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;

function TBLFontFace.GetIsSymbolFont: Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and BL_FONT_FACE_FLAG_SYMBOL_FONT) <> 0;//((FHandle.impl.faceInfo.faceFlags and BL_FONT_FACE_FLAG_SYMBOL_FONT) <> 0);
end;

function TBLFontFace.GetOutlineType: TBLFontOutlineType;
begin
  Result := TBLFontOutlineType(GetFaceInfo().OutlineType);//TBLFontOutlineType(FHandle.impl.faceInfo.outlineType);
end;

function TBLFontFace.GetPanose: BLFontPanose;//TBLFontPanose;
begin
  Result := ftGetblFontFacePanose(@FHandle)^;//Result := ;//TBLFontPanose(FHandle.impl.panose);
end;
...
var
  ...
  data: PUtf8Char;
begin
  data := ftGetblFontFacepostScriptName(@FHandle, @Sz);
  SetString(S, data, sz);  //  SetString(S, FHandle.impl.fullName.impl.data, FHandle.impl.fullName.impl.size);
  Result := String(S);     //  Result := String(S);
end;
...
begin
  Result := TBLFontStretch(ftGetblFontFaceStretch(@FHandle)); // TBLFontStretch(FHandle);//TBLFontStretch(FHandle.impl.stretch);
end;

function TBLFontFace.GetStyle: TBLFontStyle;
begin
  Result := TBLFontStyle(ftGetblFontFaceStyle(@FHandle)); // Result := TBLFontStyle(FHandle.impl.style);
end;
...
begin
  data := ftGetblFontFaceSubfamilyName(@FHandle, @Sz);
  SetString(S, data, sz);  //  SetString(S, FHandle.impl.subfamilyName.impl.data, FHandle.impl.subfamilyName.impl.size);
  Result := String(S);     //  Result := String(S);
end;

function TBLFontFace.GetUnicodeCoverage: TBLFontUnicodeCoverage;
begin
  blFontFaceGetUnicodeCoverage(@FHandle, @Result.FHandle); //Result.FHandle :=FHandle.impl.unicodeCoverage;
end;

function TBLFontFace.GetUniqueId: UInt64;
begin
  Result := ftGetblFontFaceUniqueID(@FHandle)  ;//FHandle.impl.faceUniqueId;
end;

function TBLFontFace.GetUnitsPerEm: Integer;
begin
  Result := ftGetblFontFaceUnitsPerEm(@FHandle)//FHandle.impl.designMetrics.unitsPerEm;
end;

function TBLFontFace.GetWeight: TBLFontWeight;
begin
  Result := TBLFontWeight(ftGetblFontFaceWeigth(@FHandle)); //TBLFontWeight(FHandle.impl.weight);
end;

function TBLFontFace.HasFaceFlag(const AFlag: TBLFontFaceFlag): Boolean;
begin
  Result := (Cardinal(TBLFontFaceInfo(GetFaceInfo()).GetFaceFlags) and Ord(AFlag)) <> 0;//((FHandle.impl.faceInfo.faceFlags and Ord(AFlag)) <> 0);
end;

procedure TBLFontFace.InitializeFromData(const AFontData: IBLFontData; const AFaceIndex: Integer);
begin
  FData := AFontData;
...
procedure TBLFontFace.InitializeFromFile(const AFilename: String; const AReadFlags: TBLFileReadFlags);
begin
  self.Reset;
  //FHandle := ftNewblFontFaceCore()^;
  _BLCheck(blFontFaceCreateFromFile(@FHandle, MarshaledAString(UTF8String(AFilename)), Byte(AReadFlags)));
end;
...
procedure TBLFont.ApplyGPos(const AGlyphBuffer: IBLGlyphBuffer; const AIndex: Integer; const ALookups: TBLBitWord);
begin
  if (AGlyphBuffer <> nil) then
    _BLCheck(blFontApplyGPos(@FHandle, AGlyphBuffer.Handle, AIndex, ALookups));
end;

procedure TBLFont.ApplyGSub(const AGlyphBuffer: IBLGlyphBuffer; const AIndex: Integer; const ALookups: TBLBitWord);
begin
  if (AGlyphBuffer <> nil) then
...
begin
  blFontGetDesignMetrics(@FHandle, @Result.FHandle);//Result.FHandle := FHandle.impl.face.impl.designMetrics;
end;
...
begin
  Result := TBLFontFaceFlags(ftGetblFontFaceFlags(@FHandle)); //TBLFontFaceFlags(FHandle.impl.face.impl.faceInfo.faceFlags);
end;

function TBLFont.GetFaceType: TBLFontFaceType;
begin
  Result := TBLFontFaceType(ftGetblFontFaceType(@FHandle));//TBLFontFaceType(FHandle.impl.face.impl.faceInfo.faceType);
end;

function TBLFont.GetFeatures: TArray<TBLFontFeature>;
var
 fsz: NativeUInt;
 pb : PBLFontFeatureItem;

begin
  //Result := TBLUtils.BLArrayToArray<TBLFontFeature>(FHandle.impl.features);
  Result := Nil;
  pb := ftGetblFontFeatureSettingsCoreData(@FHandle, @fsz);
  SetLength(Result, fsz);
  Move(pb^, Result[0], fsz * SizeOf(BLFontFeatureItem));
end;
...
function TBLFont.GetGlyphOutlines(const AGlyphId: Cardinal; const AUserMatrix: TBLMatrix2D;
...
begin
  ...
  _BLCheck(blFontGetGlyphOutlines(@FHandle, AGlyphId, @AUserMatrix, Result.Handle, Sink, Closure));
end;

function TBLFont.GetGlyphOutlines(const AGlyphId: Cardinal; const ASink: TBLPathSinkEvent): IBLPath;
var
  Sink: BLPathSinkFunc;
  Closure: Pointer;
begin
  ...
  _BLCheck(blFontGetGlyphOutlines(@FHandle, AGlyphId, nil, Result.Handle, Sink, Closure));
end;
...
begin
  ...
  _BLCheck(blFontGetGlyphRunOutlines(@FHandle, @AGlyphRun, @AUserMatrix, Result.Handle, Sink, Closure));
end;
...
begin
  Result := (PBLFontCore(@FHandle) <> Nil);//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;

function TBLFont.GetMatrix: TBLFontMatrix;
begin
  //Result.FHandle := FHandle.impl.matrix;
  blFontGetMatrix(@FHandle, @Result.FHandle);
end;

function TBLFont.GetMetrics: TBLFontMetrics;
begin
  //Result.FHandle := FHandle.impl.metrics;
  blFontGetMetrics(@FHandle, @Result.FHandle);
end;

function TBLFont.GetSize: Single;
begin
  Result := Self.GetMetrics.FHandle.size;//FHandle.impl.metrics.size;
end;

function TBLFont.GetStretch: TBLFontStretch;
begin
  Result := TBLFontStretch(ftGetblFontStretch(@FHandle)); // Result := TBLFontStretch(FHandle.impl.stretch);
end;

function TBLFont.GetStyle: TBLFontStyle;
begin
  Result := TBLFontStyle(ftGetblFontStyle(@FHandle));     // Result := TBLFontStyle(FHandle.impl.style);
end;

function TBLFont.GetTextMetrics( const AGlyphBuffer: IBLGlyphBuffer): TBLTextMetrics;
begin
  if (AGlyphBuffer <> nil) then
...
begin
  Result := GetDesignMetrics.FHandle.unitsPerEm;   //Result := FHandle.impl.face.impl.designMetrics.unitsPerEm;
end;

function TBLFont.GetVariations: TArray<TBLFontVariation>;
begin
  Result := Nil;//TBLUtils.BLArrayToArray<TBLFontVariation>(FHandle.impl.variations);
end;

function TBLFont.GetWeight: TBLFontWeight;
begin
 Result := TBLFontWeight(ftGetblFontWeigth(@FHandle)); //  Result := TBLFontWeight(FHandle.impl.weight);
end;

procedure TBLFont.InitializeFromFace(const AFace: IBLFontFace; const ASize: Single);
begin
...
procedure TBLFont.MapTextToGlyphs(const AGlyphBuffer: IBLGlyphBuffer; out AState: TBLGlyphMappingState);
begin
  if (AGlyphBuffer <> nil) then
    _BLCheck(blFontMapTextToGlyphs(@FHandle, AGlyphBuffer.Handle, @AState.FHandle));
end;

procedure TBLFont.PositionGlyphs(const AGlyphBuffer: IBLGlyphBuffer; const APositioningFlags: Cardinal);
begin
  if (AGlyphBuffer <> nil) then
...
class operator TBLContextCookie.Equal(const ALeft, ARight: TBLContextCookie): Boolean;
begin
  Result := ALeft.Equals(ARight);
end;
...
class operator TBLContextCookie.NotEqual(const ALeft, ARight: TBLContextCookie): Boolean;
begin
  Result := not ALeft.Equals(ARight);
end;
...
procedure TBLContext.BlitImage(const ADst: TBLRect; const ASrc: IBLImage; const ASrcArea: TBLRectI);
begin
...
procedure TBLContext.BlitImage(const ADst: TBLRectI; const ASrc: IBLImage; const ASrcArea: TBLRectI);
begin
...
constructor TBLContext.Create(const ATarget: IBLImage; const AOptions: TBLContextCreateInfo);
begin
  inherited Create;
  if (ATarget = nil) then begin
    FHandle := ftNewblContextCore()^;
    blContextInit(@FHandle);
    //_BLCheck(blContextInitAs(@FHandle, Nil, @AOptions.FHandle));
  end else begin
     FHandle := ftNewblContextCore()^;
...
begin
  if (Obj = nil) then
    ...
    Result := ftGetblContextCoreEquals(@FHandle, @TBLContext(Obj).FHandle)//(FHandle = TBLContext(Obj).FHandle)
  else
    Result := False;
end;
...
procedure TBLContext.FillGlyphRun(const ADst: TBLPoint; const AFont: IBLFont; const AGlyphRun: TBLGlyphRun);
var
  Font: PBLFontCore;
begin
  if (AFont = nil) then
  ...
  _BLCheck(blContextFillGlyphRunD(@FHandle, @ADst, Font, @AGlyphRun.FHandle));
end;

procedure TBLContext.FillGlyphRun(const ADst: TBLPoint; const AFont: IBLFont; const AGlyphRun: _PBLGlyphRun);
var
  Font: PBLFontCore;
begin
  if (AFont = nil) then
...
procedure TBLContext.FillGlyphRun(const ADst: TBLPointI; const AFont: IBLFont; const AGlyphRun: TBLGlyphRun);
var
  Font: PBLFontCore;
begin
  if (AFont = nil) then
  ...
  _BLCheck(blContextFillGlyphRunI(@FHandle, @ADst, Font, @AGlyphRun.FHandle));
end;

procedure TBLContext.FillGlyphRun(const ADst: TBLPoint; const AFont: IBLFont; const AGlyphRun: PBLGlyphRun);
var
  Font: PBLFontCore;
begin
  if (AGlyphRun <> nil) then begin
    if (AFont = nil) then
      Font := nil
    else
      Font := AFont.Handle;

    _BLCheck(blContextFillGlyphRunD(@FHandle, @ADst, Font, Pointer(@AGlyphRun^.FHandle)));
  end;
end;

procedure TBLContext.FillGlyphRun(const ADst: TBLPointI; const AFont: IBLFont; const AGlyphRun: PBLGlyphRun);
var
  Font: PBLFontCore;
begin
...
procedure TBLContext.FillText(const ADst: TBLPointI; const AFont: IBLFont; const AText: UTF8String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
...
procedure TBLContext.FillText(const ADst: TBLPoint; const AFont: IBLFont; const AText: String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
...
procedure TBLContext.FillText(const ADst: TBLPointI; const AFont: IBLFont; const AText: String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
...
procedure TBLContext.FillText(const ADst: TBLPoint; const AFont: IBLFont; const AText: UCS4String);
var
  Font: PBLFontCore;
begin
...
procedure TBLContext.FillText(const ADst: TBLPointI; const AFont: IBLFont; const AText: UCS4String);
var
  Font: PBLFontCore;
begin
  if (AText <> nil) then begin
...
procedure TBLContext.FillText(const ADst: TBLPoint; const AFont: IBLFont; const AText: UTF8String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
...
begin
  Result.FHandle := BLApproximationOptions(ftGetblContextState(@FHandle).approximationOptions);//FHandle.impl.state.approximationOptions;
end;

function TBLContext.GetCompOp: TBLCompOp;
begin
  Result := TBLCompOp(ftGetblContextState(@FHandle).CompOp);//TBLCompOp(FHandle.impl.state.compOp);
end;

function TBLContext.GetContextType: TBLContextType;
begin
  Result := TBLContextType(ftGetblContextType(@FHandle));//TBLContextType(FHandle.impl.contextType);
end;

function TBLContext.GetFillAlpha: Double;
begin
  Result := ftGetblContextState(@FHandle).styleAlpha[BL_CONTEXT_OP_TYPE_FILL];//FHandle.impl.state.styleAlpha[BL_CONTEXT_OP_TYPE_FILL];
end;

function TBLContext.GetFillColor: TBLRgba32;
begin
  _BLCheck(blContextGetFillStyle(@FHandle, @Result));
end;
...
begin
  Result := TBLFillRule(ftGetblContextState(@FHandle).fillRule);//TBLFillRule(FHandle.impl.state.fillRule);
end;

procedure TBLContext.GetFillStyle(out ARgba: TBLRgba32);
begin
  _BLCheck(blContextGetStrokeStyle(@FHandle, @ARgba.FHandle.value));
end;
...
begin
  Result := TBLStyleType(ftGetblContextState(@FHandle).styleType[BL_CONTEXT_OP_TYPE_FILL]);//TBLStyleType(FHandle.impl.state.styleType[BL_CONTEXT_OP_TYPE_FILL]);
end;

function TBLContext.GetFlattenMode: TBLFlattenMode;
begin
  Result := TBLFlattenMode(ftGetblContextState(@FHandle).approximationOptions.flattenMode);//TBLFlattenMode(FHandle.impl.state.approximationOptions.flattenMode);
end;

function TBLContext.GetFlattenTolerance: Double;
begin
  Result := ftGetblContextState(@FHandle).approximationOptions.flattenTolerance;//FHandle.impl.state.approximationOptions.flattenTolerance;
end;

function TBLContext.GetGlobalAlpha: Double;
begin
  Result := ftGetblContextState(@FHandle).globalAlpha;//FHandle.impl.state.globalAlpha;
end;

function TBLContext.GetGradientQuality: TBLGradientQuality;
begin
  Result := TBLGradientQuality(ftGetblContextState(@FHandle).hints.gradientQuality);//TBLGradientQuality(FHandle.impl.state.hints.gradientQuality);
end;
...
function TBLContext.GetState: TBLContextState;
begin
  Result.FHandle := ftGetblContextState(@FHandle)^;
end;

function TBLContext.GetHints: TBLContextHints;
begin
  Result.FHandle := (ftGetblContextState(@FHandle).hints);//FHandle.impl.state.hints;
end;

function TBLContext.GetIsNone: Boolean;
begin
  Result := PBlContextCore(@FHandle) <> nil;//((FHandle.impl.implTraits and BL_IMPL_TRAIT_NULL) <> 0);
end;

function TBLContext.GetMetaMatrix: TBLMatrix2D;
begin
  Result.FHandle := ftGetblContextState(@FHandle).metaMatrix;//FHandle.impl.state.metaMatrix;
end;

function TBLContext.GetPatternQuality: TBLPatternQuality;
begin
  Result := TBLPatternQuality(ftGetblContextState(@FHandle).hints.patternQuality);//TBLPatternQuality(FHandle.impl.state.hints.patternQuality);
end;

function TBLContext.GetRenderingQuality: TBLRenderingQuality;
begin
  Result := TBLRenderingQuality(ftGetblContextState(@FHandle).hints.renderingQuality);//TBLRenderingQuality(FHandle.impl.state.hints.renderingQuality);
end;

function TBLContext.GetSavedStateCount: Integer;
begin
  Result := Integer(ftGetblContextState(@FHandle).savedStateCount);//FHandle.impl.state.savedStateCount;
end;

function TBLContext.GetStrokeAlpha: Double;
begin
  Result := ftGetblContextState(@FHandle).styleAlpha[BL_CONTEXT_OP_TYPE_STROKE];//FHandle.impl.state.styleAlpha[BL_CONTEXT_OP_TYPE_STROKE];
end;

function TBLContext.GetStrokeColor: TBLRgba32;
begin
  _BLCheck(blContextGetStrokeStyle( @FHandle, @(TBLRgba32(Result))) );
end;

function TBLContext.GetStrokeColor64: TBLRgba64;
begin
  _BLCheck(blContextGetStrokeStyle( @FHandle, @(TBLRgba64(Result))) );
end;

function TBLContext.GetStrokeDashArray: TArray<Double>;
begin

  Result := TBLUtils.BLArrayToArray<Double>(GetState.FHandle.strokeOptions.dashArray);
...
begin
  Result := GetState.FHandle.strokeOptions.dashOffset;//FHandle.impl.state.strokeOptions.dashOffset;
end;

function TBLContext.GetStrokeEndCap: TBLStrokeCap;
begin
  Result := TBLStrokeCap(GetState.FHandle.strokeOptions.options.endCap);//TBLStrokeCap(FHandle.impl.state.strokeOptions.options.endCap);
end;
...
begin
  Result := TBLStrokeJoin(GetState.FHandle.strokeOptions.options.join);//TBLStrokeJoin(FHandle.impl.state.strokeOptions.options.join);
end;

function TBLContext.GetStrokeMiterLimit: Double;
begin
  Result := GetState.FHandle.strokeOptions.miterLimit;//FHandle.impl.state.strokeOptions.miterLimit;
end;

function TBLContext.GetStrokeOptions: TBLStrokeOptions;
begin
  Result.FHandle := GetState.FHandle.strokeOptions;//FHandle.impl.state.strokeOptions;
end;
...
begin
  Result := TBLStrokeCap(GetState.FHandle.strokeOptions.options.startCap);//TBLStrokeCap(FHandle.impl.state.strokeOptions.options.startCap);
end;
...
begin
  Result := TBLStyleType(GetState.FHandle.styleType[BL_CONTEXT_OP_TYPE_STROKE]);//TBLStyleType(FHandle.impl.state.styleType[BL_CONTEXT_OP_TYPE_STROKE]);
end;

function TBLContext.GetStrokeTransformOrder: TBLStrokeTransformOrder;
begin
  Result := TBLStrokeTransformOrder(GetState.FHandle.strokeOptions.options.transformOrder);//TBLStrokeTransformOrder(FHandle.impl.state.strokeOptions.options.transformOrder);
end;

function TBLContext.GetStrokeWidth: Double;
begin
  Result := GetState.FHandle.strokeOptions.width;//FHandle.impl.state.strokeOptions.width;
end;

function TBLContext.GetTargetHeight: Double;
begin
  Result := GetState.FHandle.targetSize.h;//FHandle.impl.targetSize.h;
end;

function TBLContext.GetTargetSize: TBLSize;
begin
  Result.FHandle := GetState.FHandle.targetSize;//FHandle.impl.targetSize;
end;

function TBLContext.GetTargetWidth: Double;
begin
  Result := GetState.FHandle.targetSize.w;//FHandle.impl.targetSize.w;
end;

function TBLContext.GetUserMatrix: TBLMatrix2D;
begin
  Result.FHandle := GetState.FHandle.userMatrix;//FHandle.impl.state.userMatrix;
end;
...
procedure TBLContext.PostRotate(const AAngle: Double; const APoint: TBLPointI);
var
  Data: array [0..2] of Double;
begin
  Data[0] := AAngle;
...
procedure TBLContext.SetStrokeTransformOrder(const AValue: TBLStrokeTransformOrder);
begin
...
procedure TBLContext.Start(const AImage: IBLImage; const AOptions: TBLContextCreateInfo);
begin
  if (AImage <> nil) then
...
procedure TBLContext.StrokeBoxArray(const ABoxes: PBLBoxI; const ACount: Integer);
var
  View: TBLArrayView<TBLBoxI>;
begin
  View.Reset(ABoxes, ACount);
...
procedure TBLContext.StrokeGlyphRun(const ADst: TBLPointI; const AFont: IBLFont; const AGlyphRun: TBLGlyphRun);
var
  Font: PBLFontCore;
begin
  if (AFont = nil) then
...
procedure TBLContext.StrokeGlyphRun(const ADst: TBLPoint; const AFont: IBLFont; const AGlyphRun: TBLGlyphRun);
var
  Font: PBLFontCore;
begin
  if (AFont = nil) then
...
procedure TBLContext.StrokePolygon(const APoly: PBLPoint; const ACount: Integer);
var
  View: TBLArrayView<TBLPoint>;
begin
  View.Reset(APoly, ACount);
...
procedure TBLContext.StrokePolygon(const APoly: PBLPointI; const ACount: Integer);
var
  View: TBLArrayView<TBLPointI>;
begin
  View.Reset(APoly, ACount);
...
procedure TBLContext.StrokeText(const ADst: TBLPoint; const AFont: IBLFont; const AText: UCS4String);
var
  Font: PBLFontCore;
begin
  if (AText <> nil) then begin
...
procedure TBLContext.StrokeText(const ADst: TBLPoint; const AFont: IBLFont; const AText: UTF8String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
...
procedure TBLContext.StrokeText(const ADst: TBLPointI; const AFont: IBLFont; const AText: UCS4String);
var
  Font: PBLFontCore;
begin
  if (AText <> nil) then begin
...
procedure TBLContext.StrokeText(const ADst: TBLPointI; const AFont: IBLFont; const AText: UTF8String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
...
procedure TBLContext.StrokeText(const ADst: TBLPoint; const AFont: IBLFont; const AText: String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
...
procedure TBLContext.StrokeText(const ADst: TBLPointI; const AFont: IBLFont; const AText: String);
var
  Font: PBLFontCore;
begin
  if (AText <> '') then begin
