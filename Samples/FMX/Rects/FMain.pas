...
begin
  ...
  W := PaintBox.Width;
  H := PaintBox.Height;
  for I := 0 to Length(FCoords) - 1 do begin
    Vertex := @FCoords[I];
    Step := @FSteps[I];

    Vertex^ := Vertex^ + Step^;

    if (Vertex.X-HALF_SIZE <= 0) or (Vertex.X+HALF_SIZE >= W) then begin
      Step.X := -Step.X;
      Vertex.X := Min(Vertex.X + Step.X, W);
    end;

    if (Vertex.Y-HALF_SIZE <= 0) or (Vertex.Y+HALF_SIZE >= H) then begin
      Step.Y := -Step.Y;
...
begin
  ...
  while (I < AValue) do begin
    FCoords[I] := PointF(Max(0,Random * W+HALF_SIZE), Max(0,Random * H+HALF_SIZE));
