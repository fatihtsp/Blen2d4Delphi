...
type
  ...
  private const
    ...
    RADIUS_SCALE   = 6;
    CATEGORY_COUNT = 3;
    COLORS: array [0..CATEGORY_COUNT - 1] of TAlphaColor = ($FFFF7F00, $FFFF3F9F, $FF7F4FFF);
  private
...
begin
  ...
  for I := 0 to N - 1 do begin
    if (FParticles.Count >= MaxParticles) then Break;
...
begin
  ...
  if (CheckBoxColors.IsChecked) then begin
    for I := 0 to CATEGORY_COUNT - 1 do Paths[I] := TBLPath.Create;

    for I := 0 to FParticles.Count - 1 do begin
      P := FParticles[I];
    ...
    for I := 0 to CATEGORY_COUNT - 1 do begin
