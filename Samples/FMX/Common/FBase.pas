...
begin
  ...
  if (CheckBoxLimitFPS.IsChecked) then begin
...
begin
  if (FBitmap = nil) or (ComboBoxRenderer.Selected = nil) then Exit;

  DstSize.Width := Trunc(PaintBox.Width);
  DstSize.Height := Trunc(PaintBox.Height);
  if (DstSize <> FDstSize) then begin
    FDstSize := DstSize;
    FPixelScale := Handle.Scale;
  ...
  Inc(FFrameCount);
  if (FTicks >= FNextSecond) then begin
...
begin
  ...
  try
    if (Data.Data <> FBlend2DBitmapData.Data) or (Data.Pitch <> FBlend2DBitmapData.Pitch) then begin
    ...
    var Aopt : TBLContextCreateInfo;
    Aopt.ThreadCount := 1;
    Aopt.Flags       := [TBLContextCreateFlag.ForceThreads,  TBLContextCreateFlag.FallbackToSync];

    var blci: TBLContextCreateInfo;
    blci.ThreadCount := 2;
    blci.Flags := [TBLContextCreateFlag.ForceThreads];
    FBlend2DContext.Start(FBlend2DImage, blci);
    try
...
begin
  if (FBitmap.Map(TMapAccess.Write, Data)) then
  try
    if (Data.Data <> FSkiaBitmapData.Data) or (Data.Pitch <> FSkiaBitmapData.Pitch) then begin
      FSkiaCanvas := nil;
      FSkiaSurface := nil;
